...
class GravityModel(QObject):
    ...
    def connect_signals(self):
        ...
        connect_once(self.ui_widget.export_request, lambda *data: self.export(*data))
    ...
    def feature_selection(self, selection_ids):
    ...
    def go(self, input_data):
        ...
        def create_graduated_symbol(field_name: str, layer: QgsVectorLayer, n_classes: int, min_size: int, max_size: int, color: QColor):
            # Create a graduated symbol renderer
            graduated_symbol = QgsGraduatedSymbolRenderer(field_name)
        
            # Update classes using Equal Interval method
            graduated_symbol.updateClasses(layer, QgsGraduatedSymbolRenderer.EqualInterval, n_classes)
        
            # Set the graduated method to size
        ...
        for f, f_coord, f_point in zip(features, f_coords, features_as_points):
            ...
            row = [f.id()] + [
                round(h / total_h, 4) if total_h > 0
                else 0
